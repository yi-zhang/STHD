...
def gene_lambda_by_ct(adata, ctcol='group'):
    ...
    for ct in celltypes_group:
        ...
        cellbygenecount = adata.layers['counts'][adata.obs.index.isin(
            ctbars), :]
        print(f'dimension of cell by gene count', cellbygenecount.shape)
        cellbygene = pd.DataFrame(
            cellbygenecount.todense(), index=ctbars, columns=adata.var_names)
        ...
        # normalized gene expr for this cell type.
        genenorm = genenorm.mean(axis=1)
        pdlst.append(
...
def select_ct_informative_genes_basic(genemeanpd, genemeanpdfc, exprcut=0.000125, fccut=2**0.5):
    ...
    genemeanpd_filtered = genemeanpd[mask_nomt &
                                     mask_noribo & mask_expr_rctd & mask_fc]
