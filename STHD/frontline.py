...
def sthd_neighbor_ct_count(adata):
    ...
    neighbor_celltype_name = np.array(
        ["|".join(sorted(list(n_ct.keys()))) for n_ct in tqdm(neighbor_celltypes)]
    )
...
def get_frontline(
    ...
    labels[
        np.where(adata.obs.index.isin(subset[subset.obs["label"] == 1].obs.index))[0]
    ] = 1
    labels[
        np.where(adata.obs.index.isin(subset[subset.obs["label"] == 2].obs.index))[0]
    ] = 2
    labels[
        np.where(adata.obs.index.isin(subset[subset.obs["label"] == 3].obs.index))[0]
    ] = 3
    labels[
        np.where(adata.obs.index.isin(subset[subset.obs["label"] == 4].obs.index))[0]
    ] = 4
